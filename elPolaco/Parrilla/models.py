...
class Pedido(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Si hay una mesa asociada y el pedido no está pagado, cambiar la mesa a ocupada
        if self.mesa:
            if not self.pagado and self.mesa.estado != 'ocupada':
                self.mesa.estado = 'ocupada'
                self.mesa.save()
            # Si el pedido está pagado, cambiar la mesa a libre
...
class DetallePedido(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Actualizar el total del pedido después de guardar el detalle
